    ...
    def chooseAction(self, gameState):
        ...
        for distribution in livingGhostPositionDistributions:
            ...
            for key in distribution.keys():
                if distribution[key] > maxProb:
                    maxProb = distribution[key]
                    mostLikelyPos = key
        ...
        for pos in mostLikelyGhostPositions:
            if self.distancer.getDistance(pacmanPosition, pos) < dist:
        ...
        dist = float("inf")        
        for action in legal:
            if self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), posBest) < dist:
                dist = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), posBest)
                bestAction = action
