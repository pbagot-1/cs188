...
class QLearningAgent(ReinforcementAgent):
    ...
    def __init__(self, **args):
        ...
        #for key in args:
        #    print ("another keyword arg: %s: %s" % (key, args[key]))
        "*** YOUR CODE HERE ***"
    ...
    def computeValueFromQValues(self, state):
        ...
        for action in self.getLegalActions(state):
            listOfVals.append(self.getQValue(state, action))
    ...
    def computeActionFromQValues(self, state):
        ...
        for item in listOfVals:
            if item == max(listOfVals):
                randomList.append(index)
            index = index + 1
        return self.getLegalActions(state)[random.choice(randomList)]
    ...
    def update(self, state, action, nextState, reward):
        ...
        self.seenStates.add((state, action))
        #util.raiseNotDefined()
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feature in self.featExtractor.getFeatures(state, action):
            sum = sum + self.weights[feature]*feature
        return sum"""
        #util.raiseNotDefined()
